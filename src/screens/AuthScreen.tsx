...
export default function AuthScreen({ navigate }: NavProps) {
    ...
    <div className="relative flex h-full flex-col overflow-hidden bg-[#08152b] px-6 text-right text-white">
      ...
      <div className="relative pt-16">
        <div className="mb-10 flex items-center justify-between">
          <div><p className="mb-2 text-sm font-bold text-cyan-300">ذاكر معي</p><h1 className="text-3xl font-black leading-tight">مستقبلك الدراسي<br />بيبدأ من هنا</h1></div>
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/[0.08] p-5 backdrop-blur-xl">
          <div className="mb-6 flex rounded-2xl bg-black/20 p-1">
            {(['signin', 'signup'] as const).map((item) => <button key={item} onClick={() => setMode(item)} className={`flex-1 rounded-xl py-3 text-sm font-black transition ${mode === item ? 'bg-white text-[#0b1a34]' : 'text-white/50'}`}>{item === 'signin' ? 'تسجيل الدخول' : 'حساب جديد'}</button>)}
          </div>
