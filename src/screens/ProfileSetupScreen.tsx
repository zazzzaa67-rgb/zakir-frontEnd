  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#F8F9FF]">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-4">
        ...
        {grade > 0 && <><p className="mb-2 text-sm font-black text-slate-800">{grade === 1 ? 'المسار' : 'الشعبة / المسار'}</p><div className="grid grid-cols-1 gap-2">{tracks[grade as 1 | 2 | 3].map(track => <button key={track.id} onClick={() => setTrackId(track.id)} className={`rounded-2xl border-2 p-4 text-right font-black ${trackId === track.id ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-slate-100 bg-white text-slate-700'}`}>{track.label}<span className="float-left">{trackId === track.id ? '✓' : '○'}</span></button>)}</div></>}
