...
export default function VideoPlayerScreen({ navigate }: NavProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#0A0F1E]">
      {/* Video area */}
      <div className="relative" style={{ aspectRatio: '16/9' }}>
        ...
        >
          ...
          <div className="absolute inset-0 opacity-10">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="absolute w-full h-px bg-blue-400" style={{ top: `${(i + 1) * 16.66}%` }} />
            ))}
          </div>
        ...
        <div className="absolute top-0 left-0 right-0 flex items-center justify-between p-4">
          ...
          <button onClick={() => navigate('ai_lesson')} className="w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-white font-bold">
            ×
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 bg-[#F8F9FF] flex flex-col overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-slate-100">
          <h2 className="text-lg font-black text-slate-900 text-right mb-1">المعادلات الخطية</h2>
          <div className="flex items-center gap-3">
            <span className="text-slate-500 text-sm font-medium">12 / 25 دقيقة</span>
            <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
              <div
                className="h-full rounded-full transition-all"
                style={{ width: `${progress}%`, background: 'linear-gradient(90deg, #1E6FF0, #7C3AED)' }}
              />
            </div>
        ...
        <div className="px-5 py-2">
          <input
            ...
            onChange={(e) => {
              ...
              if (+e.target.value >= 100) setShowComplete(true);
            }}
        ...
        {showTranscript && (
          <div className="flex-1 overflow-y-auto px-5 pb-4">
            <div className="bg-white rounded-2xl p-4" style={{ boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
              <h3 className="font-black text-slate-900 text-sm mb-3 text-right">نص الشرح</h3>
              <p className="text-slate-600 text-sm leading-relaxed text-right font-medium">
                المعادلة الخطية هي معادلة من الدرجة الأولى بمتغير واحد أو أكثر. الشكل العام هو ax + b = c حيث a وb وc أعداد ثابتة وa ≠ 0. لحل المعادلة، نقوم بعزل المتغير x من جهة واحدة...
              </p>
            </div>
          </div>
        )}
      ...
      {showComplete && (
        ...
        >
          <div
            className="bg-white rounded-3xl p-8 mx-8 text-center"
            style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}
          >
            <div className="text-6xl mb-4">🎉</div>
            <h2 className="text-2xl font-black text-slate-900 mb-2">أحسنت!</h2>
            <p className="text-slate-600 font-medium mb-4">خلصت درس المعادلات الخطية</p>
            <div
              className="text-3xl font-black mb-6"
              style={{ background: 'linear-gradient(135deg, #1E6FF0, #7C3AED)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}
            >
              +20 Points 🏆
            </div>
            <div className="flex flex-col gap-3">
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #1E6FF0, #7C3AED)', boxShadow: '0 8px 24px rgba(30,111,240,0.4)' }}
              >
                اختبر نفسك 🧠
              </button>
              <button
                onClick={() => navigate('summary')}
                className="w-full py-3.5 rounded-2xl font-bold text-blue-600"
